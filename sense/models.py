...
class SensePost(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='sense_posts', editable=False)
    ...
    view = models.BigIntegerField(default=0, editable=False)  
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            if SensePost.objects.filter(slug=slug_sample).exists():
                # Generate a random alphanumeric string of length 6
                random_string = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
                slug_sample = slugify(self.title + ' ' + random_string)
            self.slug = slug_sample
        super(SensePost, self).save(*args, **kwargs)
        
    
    
    
    def get_absolute_url(self):
        return reverse('sense:sense_detail', args=[self.slug])
